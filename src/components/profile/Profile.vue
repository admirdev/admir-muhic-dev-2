<template>
  <div class="row align-items-center gx-5">
    <div class="col-xxl-5">
      <div class="text-center text-xxl-start">
        <div class="fs-3 fw-light text-muted">
          En applikation byggd med Vue.js och .NET
        </div>
        <h1 class="display-3 fw-bolder mb-5">
          <span class="text-dark d-inline">
            Kolla in mina hobbyprojekt och mitt CV!
          </span>
        </h1>
        <div
          class="d-grid gap-3 d-sm-flex justify-content-sm-center justify-content-xxl-start mb-3"
        >
          <router-link
            class="btn btn btn-outline-dark btn-lg px-5 py-3 me-sm-3 fs-6 fw-bolder"
            to="/projects"
          >
            Projekt <i class="bi bi-chevron-right"></i>
          </router-link>
          <router-link
            class="btn btn-default btn-lg px-5 py-3 fs-6 fw-bolder"
            to="/resume"
          >
            Curriculum vitae <i class="bi bi-chevron-right"></i>
          </router-link>
        </div>
      </div>
    </div>

    <div class="col-xxl-7">
      <div
        class="d-flex justify-content-center justify-content-xxl-end mt-5 mt-xxl-0"
      >
        <div class="bg-light rounded-5 p-2">
          <img
            class="img-fluid"
            src="@/assets/profile.png"
            alt="Profile picture"
          />
        </div>
      </div>
    </div>
  </div>
</template>

<script setup></script>

<style scoped lang="scss">
@import "@/styles/components/_default-btn.scss";
</style>
